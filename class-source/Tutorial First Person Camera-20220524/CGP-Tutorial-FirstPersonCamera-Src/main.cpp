...
// Tutorial: First Person Camera
// Here we will look at how to impliment a simple first person vector camera, to make it look 
// like you are walking around. We will also learn how to go fullscreen, and exit without 
// getting errors.
// In this lesson, we will be learning a way to impliment a simple first person camera, which will 
// enable us to look up, down, left, and right with the mouse, and move forward and back, and 
// strafe left and right. It's not too difficult, so i'll just get right to the code and explain 
// whats going on as we go. We will be using a seamless grass texture I made to texture the ground 
// with grass.
...
XMVECTOR camUp;
///////////////**************new**************////////////////////
// Here we have a bunch of new declarations. You can see four new vectors, two new matrices, 
// and a couple floats. The first two vectors are describing the forward and right direction 
// in the world. We will use these when calculating the rotation of our camera. The second 
// two vectors are the forward and right directions of our camera. We will use them to move 
// our camera around, so when we press the forward key, all we have to do is move along the 
// camForward vector to make it look like our camera is walking straigt ahead.
// The camRotationMatrix doesn't actually need to be created, since we could have just used 
// the same rotation matrix we have been using for our cubes, but I just made it anyway. 
// We will use it to rotate our camera! The second one is another matrix describing the world 
// space of our "ground". Again, we could have used one of the cube world matrices, but i 
// thought this would be more clear.
// The first two floats, moveLeftRight and moveBackForward, will be used to move along the 
// camForward and camRight vectors when we want to move the camera forward or strafe right.
// The second two floats are used to calculate the rotation around the x and y axis for our 
// camera when looking around.
...
{	
	...
	///////////////**************new**************////////////////////
	// Here is our swapchain description. To go fullscreen, we have to set the Windowed member 
	// to false. We can go in and out of fullscreen by pressing "Alt+Enter".
...
///////////////**************new**************////////////////////
// Here is our UpdateCamera() function, which will be called every frame at the 
// end of our DetectInput() function.
void UpdateCamera()
{
	// Rotating the Camera
	//
...
// We have updated our DetectInput() function to update our camera when moving the mouse or pressing the 
// W, S, A, or D keys. Notice the line before we start checking for input. We have a new variable called
// "speed". This is the speed our camera will move when we reposition it every frame. We take the time 
// variable passed in when this function is called, and multiply it with our speed, so that our camera 
// will move the exact same distance in one second whatever the frames per second are. Next we check if 
// the W, S, A, and D keys were pressed, and update the moveLeftRight and moveBackForward accordingly. 
// After that we check for the mouse input, where we will update the camYaw and camPitch depending on how 
// much the mouse has moved in the x or y axis since the last frame. After all that, we update our camera 
// by calling the UpdateCamera() function.
void DetectInput(double time)
{
...
///////////////**************new**************////////////////////
// Since Direct3D can't exit properly from fullscreen directly, we need to do something extra before 
// we actually exit. We need to take our application out of fullscreen and into windowed mode before 
// we actually start cleaning up. We can do that in the CleanUp() function by calling the 
// SetFullScreenState() method of our swapchain.
void CleanUp()
{
	///////////////**************new**************////////////////////
...
//	hr = D3DX11CompileFromFile(L"Effects.fx", 0, 0, "D2D_PS", "ps_4_0", 0, 0, 0, &D2D_PS_Buffer, 0, 0);
	hr = D3DCompileFromFile(L"Effects.fx", 0, 0, "VS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &VS_Buffer, 0);
	hr = D3DCompileFromFile(L"Effects.fx", 0, 0, "PS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &PS_Buffer, 0);
	hr = D3DCompileFromFile(L"Effects.fx", 0, 0, "D2D_PS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &D2D_PS_Buffer, 0);
	...
	// 
	// Of course we don't have to do this, but to get the full color of our grass texture, we need to make sure 
	// the light is hitting it directly from above.
	...
	// texture(I created a seamless grass texture for this), instead of one very stretched out texture covering 
	// the whole ground.
...
//	hr = D3DX11CreateShaderResourceViewFromFile( d3d11Device, L"grass.jpg", NULL, NULL, &CubesTexture, NULL);
	hr = CreateDDSTextureFromFile(d3d11Device, L"grass.dds", NULL, &CubesTexture);
